...
public abstract class BasicMCPIT {
    ...
    public void readTemplatedResource() throws InterruptedException {
        ...
        do {
            attempts++;
            logs = client().getLogs();
            logLines = logs.stream()
                    .filter(line -> line.contains("MCP logger"))
                    .map(line -> line.replaceAll("[^\\x00-\\xFF]", ""))
                    .toList();
            Thread.sleep(1000);
        ...
        Assertions.assertTrue(log.contains("MCP logger: resource_template:fileresource:"),
                "Log message doesn't contain autogenerated prefix: " + log);
        Assertions.assertTrue(log.contains("Reading provided file"),
                "Log message doesn't contain user message prefix: " + log);
        Assertions.assertTrue(log.endsWith("robot-readable.txt\""),
                "Log message doesn't contain file name:" + log);
    }
